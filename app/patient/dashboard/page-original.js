  ...
  const loadDashboard = async () => {
    try {
      ...
      if (isActive) {
        ...
        const { data: pending } = await supabase.from('payments').select('id').eq('patient_id', patientData.id).eq('payment_status', 'pending_verification').maybeSingle();
        setSubscriptionStatus(pending ? 'pending' : 'expired');
      }
        ...
        <header className="flex justify-between items-center mb-10">
          ...
          {assessment ? (
            ...
            <div className="text-center py-6">
              ...
              <button
                onClick={() => router.push('/patient/assessment')}
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-2xl font-bold shadow-lg shadow-indigo-600/20 transition-all"
              >
                Start Assessment
              </button>
            </div>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              className="p-4 bg-slate-50 hover:bg-violet-50 border border-slate-100 hover:border-violet-200 rounded-2xl transition-all group text-left"
            >
              <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center mb-3 shadow-sm group-hover:scale-110 transition-transform">
      ...
      {reminders.length > 0 ? (
        ...
        <div className="bg-amber-50 p-6 rounded-2xl border border-amber-100">
          ...
          <p className="text-sm text-amber-800 leading-relaxed">
            No reminders set. Remember to drink 8 glasses of water today and take a 30-minute walk!
          </p>
          <button className="mt-4 text-xs font-bold text-amber-700 hover:text-amber-900 uppercase tracking-wide">
            + Set Reminder
          </button>
        </div>
      )}
