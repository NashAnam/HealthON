...
export const notifyAppointmentBooked = (appointment, doctor) => {
    ...
    const body = `Great choice! Dr. ${doctorName} is ready to see you on ${appointment.appointment_date} at ${appointment.appointment_time}. Mark your calendar!`;
...
export const checkUpcomingAppointments = (appointments) => {
    ...
    appointments.forEach(appt => {
        try {
            if (!appt.appointment_date || !appt.appointment_time) return;

            // Normalize time (ensure it's HH:MM:SS)
            let timeStr = appt.appointment_time;
            if (timeStr.includes('AM') || timeStr.includes('PM')) {
                // Convert simple AM/PM to 24h if needed, or just rely on a safer parser
                // For now, let's try a safer concat
            }
            ...
            if (isNaN(apptDateTime.getTime())) {
                // Try without T or other formats if T failed
                const alternative = new Date(`${appt.appointment_date} ${appt.appointment_time}`);
                if (isNaN(alternative.getTime())) return; // Skip if still invalid
            }

            const finalDate = isNaN(apptDateTime.getTime()) ? new Date(`${appt.appointment_date} ${appt.appointment_time}`) : apptDateTime;
            ...
            if (hoursDiff > 23 && hoursDiff <= 24 && appt.status === 'confirmed') {
                ...
                const body = `Don't forget! You have an appointment with Dr. ${appt.doctors?.name || 'your doctor'} tomorrow at ${appt.appointment_time}. Get a good night's sleep!`;
            ...
            if (hoursDiff > 0 && hoursDiff <= 1 && appt.status === 'confirmed') {
                ...
                let body = `Time to get ready! Your appointment with Dr. ${appt.doctors?.name || 'your doctor'} starts in 1 hour`;
            ...
            if (hoursDiff > 0 && hoursDiff <= 0.25 && appt.consultation_type === 'telemedicine' && appt.status === 'confirmed') {
                toast((t) => (
                    <div className="flex flex-col gap-2">
                        ...
                        <p className="text-sm">Dr. {appt.doctors?.name || 'Doctor'} is waiting</p>
                        <button
                            onClick={() => {
                                window.location.href = `/patient/telemedicine/room?id=${appt.id}`;
                                toast.dismiss(t.id);
                            }}
            ...
            console.error('Check Appointment Error:', e);
        }
    });
};
...
export const notifyDoctorNewAppointment = (appointment, patient) => {
    ...
    const body = `${patientName} has booked an appointment for ${appointment.appointment_date} at ${appointment.appointment_time}. Get ready to help!`;
...
export const notifyLabTestBooked = (booking, lab) => {
    ...
    const body = `Perfect! Your ${booking.test_type} is scheduled at ${labName} for ${booking.test_date}. Stay healthy!`;
...
export const notifyLabReportReady = (report, lab) => {
    ...
    const labName = lab?.name || 'Lab';
    const body = `Good news! Your ${report.test_type} report from ${labName} is now available. Check it out!`;
